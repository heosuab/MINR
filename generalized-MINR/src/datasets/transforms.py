...
def create_transforms(config, split="train", is_eval=False):
    if config.transforms.type in ["imagenette182x182", "imagenette256x256", "imagenette512x512"]:
    ...
    elif config.transforms.type in ["indoor182x182"]:
        resolution = int(config.transforms.type.split("x")[-1])
        if split == "train" and not is_eval:
            transforms_ = [
                transforms.Resize(resolution),
    ...
    elif config.transforms.type in ["celeba182x182"]:
        resolution = int(config.transforms.type.split("x")[-1])
        transforms_ = [
            transforms.Resize(resolution),
            transforms.CenterCrop(resolution),

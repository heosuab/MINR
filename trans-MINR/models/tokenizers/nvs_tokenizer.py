...
class NvsTokenizer(nn.Module):
    def __init__(self, input_size, patch_size, dim, padding=0, img_channels=3):
        ...
        self.prefc = nn.Linear(
            patch_size[0] * patch_size[1] * (img_channels + 3 + 3), dim
        )
        self.grid_shape = (
            (input_size[0] + padding[0] * 2) // patch_size[0],
    ...
    def forward(self, data):
        ...
        B = imgs.shape[0]
        H, W = imgs.shape[-2:]
        rays_o, rays_d = poses_to_rays(
            data["support_poses"], H, W, data["support_focals"]
        )

...
def volume_rendering(
    nerf, rays_o, rays_d, near, far, points_per_ray, use_viewdirs, rand
):
    """
    Args:
    ...
    pts = rays_o.view(B, n_rays, 1, 3) + rays_d.view(B, n_rays, 1, 3) * z_vals.view(
        1, n_rays, points_per_ray, 1
    )
    ...
    # Do volume rendering
    dists = torch.cat(
        [z_vals[:, 1:] - z_vals[:, :-1], torch.ones_like(z_vals[:, -1:]) * 1e-3], dim=-1
    )  # n p

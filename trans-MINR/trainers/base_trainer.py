...
class BaseTrainer:
    def __init__(self, rank, cfg):
        ...
        if self.is_master:
            ...
            if env["wandb_upload"]:
                ...
                wandb.init(
                    project=wandb_cfg["project"], entity=wandb_cfg["entity"], config=cfg
                )
            else:
    ...
    def train(self):
        ...
        for epoch in range(1, max_epoch + 1):
            ...
            if epoch % save_epoch == 0:
                self.save_checkpoint(f"save-epoch-{epoch}.pth")

            dir = os.listdir(cfg["env"]["save_dir"])
            self.save_checkpoint(f"epoch-{epoch:04}.pth")
            if len(dir) > 6:
                ckpt_files = [_ for _ in dir if _.endswith(".pth")]
                ckpt_files.sort()
                os.remove(os.path.join(cfg["env"]["save_dir"], ckpt_files[0]))
    ...
    def sync_ave_scalars_(self, ave_scalars):
        for k in ave_scalars.keys():
            x = torch.tensor(
                ave_scalars[k].item(), dtype=torch.float32, device=self.device
            )
